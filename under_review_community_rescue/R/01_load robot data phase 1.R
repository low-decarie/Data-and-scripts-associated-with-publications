...
#and associates the data with treatment variables found in a legend file
...
file.list<-list.files("./Data/Phase 1/Without recent dispersal/",recursive=T)
...
read.file<-function(file){
    ...
    path<-paste("./Data/Phase 1/Without recent dispersal/", file, sep="")
...
#Create a plate template (rows and columns) ####
    ...
    #Merge the labels with the numeric data assuming each plate has 96 wells
...
robo.data$is.blank.plate <- grepl("Blank",robo.data$plateID)

robo.data <- ddply(.data=robo.data,
                   .variables=c("dalapon","glucose"),
                   function(x){
                     mean.blank <- mean(x$OD[x$is.blank.plate], na.rm=T)
                     sd.blank <- sd(x$OD[x$is.blank.plate], na.rm=T)
                     temp <- data.frame(x, mean.blank=mean.blank,
                                        sd.blank=sd.blank)
                     return(temp)})

# robo.data <- ddply(.data=robo.data,
#                    .variables=c("transfer","plateID"),
#                    function(x){
...
#Save files ####
write.csv(robo.data, file="./Outputs/phase 1 robot OD data.csv")
save(robo.data, file="./Outputs/phase 1 robot OD data.RData")